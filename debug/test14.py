...
def open_wikipedia():
    """Ask the user if they want to open the Wikipedia page."""
    open_wikipedia_choice = questionary.confirm("Do you want to open the Wikipedia page?").ask()
    if open_wikipedia_choice:
...
def time_elapsed():
    ...
    if time_set[0] <= 0:
        print("Time's up!")
        open_wikipedia()  # Open Wikipedia page when the timer finishes
        return
...
def start_or_restart():
    ...
    while True:
        restart_choice = input("Do you want to restart? (y/n): ").lower()
